...
const Page = () => {
  ...
  const activeLoc = data?.find((l) => l.id === activeLocation);
    ...
    <div className="pb-8 mt-[-1.5rem]">
      ...
      >
        ...
        <div className="flex gap-6">
          <div className="flex flex-col gap-4 max-h-[70vh] min-w-[200px] overflow-y-auto">
          ...
          <div className="flex flex-col gap-2 max-h-[70vh] min-w-[240px] overflow-y-auto">
            {activeLoc?.items?.concat(activeLoc?.containers)?.map((item) => {
              return item?.hasOwnProperty("parentContainerId") ? (
                <ColorCard isContainer item={item} key={v4()} />
              ) : (
                <SquareItemCard item={item} key={v4()} />
              );
            })}
          </div>
        </div>
