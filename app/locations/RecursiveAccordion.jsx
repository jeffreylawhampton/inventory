...
const RecursiveAccordion = ({ data }) => {
  ...
  return data.map((parent) => {
    return (
      <Accordion key={parent.name} className="pl-[6px] pr-0">
        <AccordionItem
          title={parent.name}
          aria-label={parent.name}
          classNames={{ titleWrapper: "bg-gray-500", content: "bg-gray-200" }}
        >
          ...
          {parent.containers && <RecursiveAccordion data={parent.containers} />}
        </AccordionItem>
      </Accordion>
    );
  });
